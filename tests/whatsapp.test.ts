...
describe('WhatsApp interactive workflow', () => {
  ...
  it('scopes jobs and rejects stale or cross-sender actions', async () => {
    ...
    let result = await command({ command: '', actionId: action(aJob), from: 'A' }); const green = action(result); result = await command({ command: '', actionId: green, from: 'A' }); await expect(command({ command: '', actionId: green, from: 'A' })).rejects.toThrow()
  ...
  it('does not fabricate non-demo reports and preserves approved custom measurements', async () => {
    ...
    result = await command({ command: 'latest leads', from: 'D' }); result = await command({ command: '', actionId: firstLeadId(formatReply(result)), from: 'D' }); result = await command({ command: '', actionId: action(result), from: 'D' }); result = await command({ command: '', actionId: action(result), from: 'D' }); result = await command({ command: '', actionId: action(result), from: 'D' }); result = await command({ command: '', actionId: action(result), from: 'D' }); result.job!.measurements!.totalAreaSqFt = 1234
...
describe('webhook interactive round trip', () => {
  ...
  it('parses list_reply and button_reply and emits native payloads', async () => {
    ...
    await send({ id: 'in2', from: 'E', type: 'interactive', interactive: { list_reply: { id: lead } } }); const second = sent.at(-1) as { interactive: { action: { buttons: Array<{ reply: { id: string } }> } } }; expect(second.interactive.action.buttons[0].reply.id).toContain('approve-outreach')
