...
export function formatReply(result: { message?: string; leads?: Array<{ address: string; city: string; state: string; zip: string; id: string }>; actions?: Array<{ id: string; title: string }>; eagleViewUrl?: string }): WhatsAppInteractive {
  if (result.leads?.length) return { type: 'interactive', interactive: { type: 'list', body: { text: (result.message ?? '').slice(0, 1024) }, action: { button: 'Choose lead', sections: [{ title: 'Roofing leads', rows: result.leads.slice(0, 10).map((lead) => ({ id: `lead:${lead.id}:select`, title: `${lead.address}, ${lead.city}`.slice(0, 24), description: `${lead.address}, ${lead.city}, ${lead.state} ${lead.zip}`.slice(0, 72) })) }] } } }
