...
export async function getLatestLeads(location: string): Promise<Lead[]> {
  ...
  if (process.env.DEMO_MODE !== '0' || !base || !process.env.PERMIT_ATLAS_API_KEY) {
    return demoLeads.filter((lead) => lead.city.toLowerCase() === location.toLowerCase() || location.toLowerCase() === 'austin')
  }
